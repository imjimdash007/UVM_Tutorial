...
// BACKDOOR ACCESS
          assert(uvm_hdl_deposit("top.dut1.r0", 112));
      //regmodel.r0.write(status, .value(112), .parent(this));
      //assert( status == UVM_IS_OK );

      regmodel.r1.write(status, .value(223), .parent(this));
      assert( status == UVM_IS_OK );

      regmodel.r0.read(status, .value(incoming), .parent(this));
      assert( status == UVM_IS_OK );
      assert( incoming == 112 )
        else `uvm_warning("", $sformatf("incoming = %d, expected = 112", incoming))

      regmodel.r1.read(status, .value(incoming), .parent(this));
      assert( status == UVM_IS_OK );
      assert( incoming == 223 )
        else `uvm_warning("", $sformatf("incoming = %d, expected = 223", incoming))
