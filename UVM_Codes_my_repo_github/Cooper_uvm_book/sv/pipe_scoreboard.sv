...
class pipe_scoreboard extends uvm_scoreboard;
   ...
   virtual task compare_data( );
      ...
      if(exp_data0 == output_packet.data_out0)
         `uvm_info(get_type_name( ), $sformatf("Actual output data %0h does match expected %0h", output_packet.data_out0, exp_data0),UVM_LOW)
      ...
      if(exp_data1 == output_packet.data_out1)
         `uvm_info(get_type_name( ), $sformatf("Actual output data %0h does match expected %0h", output_packet.data_out1, exp_data1),UVM_LOW)
