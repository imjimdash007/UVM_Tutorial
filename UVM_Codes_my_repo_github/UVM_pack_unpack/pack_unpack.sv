...
class unpack_test extends uvm_test;
 `uvm_component_utils(unpack_test)
    ...
    int  m_bit1 ,m_bytes1 ,m_ints1;
 
    rand packet m_pkt, m_pkt2; 
    ...
    task run_phase(uvm_phase phase);
      ...
      m_pkt.print();
      m_pkt.pack(m_bit);
     `uvm_info("UNPACK_TEST --> PACKING", $sformatf("m_bit =%p", m_bit), UVM_LOW)     
      m_bit1 = m_pkt2.unpack(m_bit);
      `uvm_info("UNPACK_TEST --> UNPACKING", $sformatf("m_bit1 =0x%0h", m_bit1), UVM_LOW) 

      m_pkt.randomize();
      m_pkt.print();
      m_pkt.pack_bytes(m_bytes);
     `uvm_info("UNPACK_TEST --> PACKING", $sformatf("m_bytes =%p", m_bytes), UVM_LOW) 
      m_bytes1 = m_pkt2.unpack_bytes(m_bytes);
      `uvm_info("UNPACK_TEST --> UNPACKING", $sformatf("m_bytes1 =0x%0h", m_bytes1), UVM_LOW) 

      m_pkt.randomize();
      m_pkt.print();
      m_pkt.pack_ints(m_ints);
     `uvm_info("UNPACK_TEST", $sformatf("m_ints =%p", m_ints), UVM_LOW) 
      m_ints1 = m_pkt2.unpack_ints(m_ints);
      `uvm_info("UNPACK_TEST --> UNPACKING", $sformatf("m_ints1 =0x%0h", m_ints1), UVM_LOW) 
      phase.drop_objection(this); 
    endtask
endclass
