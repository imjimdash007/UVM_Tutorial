...
function void sfr_scoreboard::write(sfr_seq_item t);
  if(t.re == 1) begin
    if(mem.exists(t.address)) begin
      reads++;
      `uvm_info("** From SBD **", $sformatf("mem[t.address] = %0d valid read scenarios", mem[t.address]), UVM_LOW)
      `uvm_info("** From SBD **", $sformatf("t.read_data    = %0d valid read scenarios", t.read_data), UVM_LOW)
